...
-- регистронезависимые email/slug
...
CREATE TABLE users (
  ...
  archived_by_id uuid REFERENCES users(id) ON DELETE
  SET
    NULL,
...
CREATE TABLE color_presets (
  ...
  CONSTRAINT ck_color_presets__bd_hex CHECK (
    border_color IS NULL
    OR border_color ~ '^#[0-9A-Fa-f]{6}$'
  ),
...
CREATE TABLE organizations (
  ...
  description text,
  logo_file_key text,
  color_preset_id uuid REFERENCES color_presets(id) ON DELETE
  SET
    NULL,
    ...
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    created_by_id uuid REFERENCES users(id) ON DELETE
  SET
    NULL,
    updated_by_id uuid REFERENCES users(id) ON DELETE
  SET
    NULL,
    archived_at timestamptz,
    archived_by_id uuid REFERENCES users(id) ON DELETE
...
ALTER TABLE
  color_presets
ADD
  CONSTRAINT fk_color_presets__org FOREIGN KEY (organization_id) REFERENCES organizations(id) ON DELETE CASCADE;
...
CREATE TABLE invitations (
  ...
  invited_by_id uuid REFERENCES users(id) ON DELETE
  SET
    NULL,
    expires_at timestamptz NOT NULL,
    accepted_at timestamptz,
    accepted_user_id uuid REFERENCES users(id) ON DELETE
  SET
    NULL,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now()
...
CREATE TABLE boards (
  ...
  SET
    ...
    owner_user_id uuid NOT NULL REFERENCES users(id) ON DELETE RESTRICT,
    responsible_team_id uuid REFERENCES teams(id) ON DELETE
  SET
    NULL,
...
CREATE TABLE columns (
  ...
  SET
    NULL,
    version int NOT NULL DEFAULT 1,
    CONSTRAINT ck_columns__task_limit CHECK (
      task_limit IS NULL
      OR task_limit > 0
    )
);
...
CREATE TABLE tasks (
  ...
  type_id uuid REFERENCES task_types(id) ON DELETE
  SET
    NULL,
    category_id uuid REFERENCES task_categories(id) ON DELETE
  SET
    NULL,
    priority_id uuid REFERENCES priorities(id) ON DELETE
  SET
    NULL,
    responsible_user_id uuid REFERENCES users(id) ON DELETE
  SET
    NULL,
  ...
  SET
    NULL,
    version int NOT NULL DEFAULT 1,
    CONSTRAINT ck_tasks__title_len CHECK (
      char_length(title) BETWEEN 1
      AND 500
    ),
    CONSTRAINT ck_tasks__date_order CHECK (
      start_date IS NULL
      OR due_date IS NULL
...
CREATE TABLE attachments (
  ...
  file_size bigint,
  external_url text,
  uploaded_by_id uuid REFERENCES users(id) ON DELETE
  SET
    NULL,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
  ...
  SET
    NULL,
    version int NOT NULL DEFAULT 1,
    CONSTRAINT ck_attachments__size CHECK (
      file_size IS NULL
      OR file_size >= 0
    ),
...
ALTER TABLE
  tasks
ADD
  CONSTRAINT fk_tasks__cover FOREIGN KEY (cover_attachment_id) REFERENCES attachments(id) ON DELETE
SET
  NULL;
...
CREATE TABLE task_assignees (
  ...
  assigned_by_id uuid REFERENCES users(id) ON DELETE
  SET
    NULL,
...
CREATE TABLE task_tags (
  ...
  -- теги архивируются, не удаляются
  added_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE checklist_items (
  ...
  completed_at timestamptz,
  completed_by_id uuid REFERENCES users(id) ON DELETE
  SET
    NULL,
    assignee_user_id uuid REFERENCES users(id) ON DELETE
  SET
    NULL,
    due_date timestamptz,
    position numeric(20, 10) NOT NULL,
...
-- ===== 5. INDEXES ===================================================
CREATE UNIQUE INDEX ux_users__email_active ON users (email)
WHERE
  archived_at IS NULL;
...
CREATE INDEX ix_pwd_reset__active ON password_reset_tokens (user_id)
WHERE
  consumed_at IS NULL;

CREATE INDEX ix_email_verif__active ON email_verification_tokens (user_id)
WHERE
  consumed_at IS NULL;

CREATE UNIQUE INDEX ux_org_members__org_user_active ON organization_members (organization_id, user_id)
WHERE
  archived_at IS NULL;

CREATE INDEX ix_org_members__user ON organization_members (user_id);

CREATE UNIQUE INDEX ux_roles__system_key ON roles (key)
WHERE
  organization_id IS NULL;

CREATE UNIQUE INDEX ux_invitations__org_email_pending ON invitations (organization_id, email)
WHERE
  status = 'pending';

CREATE INDEX ix_invitations__org ON invitations (organization_id);

CREATE UNIQUE INDEX ux_teams__org_name_active ON teams (organization_id, name)
WHERE
  archived_at IS NULL;

CREATE INDEX ix_team_members__user ON team_members (user_id);

CREATE UNIQUE INDEX ux_groups__org_name_active ON groups (organization_id, name)
WHERE
  archived_at IS NULL;

CREATE INDEX ix_group_members__user ON group_members (user_id);

CREATE UNIQUE INDEX ux_tags__org_name_active ON tags (organization_id, name)
WHERE
  archived_at IS NULL
  AND board_id IS NULL;

CREATE UNIQUE INDEX ux_tags__board_name_active ON tags (board_id, name)
WHERE
  archived_at IS NULL
  AND board_id IS NOT NULL;

CREATE INDEX ix_boards__org_active ON boards (organization_id)
WHERE
  archived_at IS NULL;
...
CREATE INDEX ix_columns__board_position ON columns (board_id, position)
WHERE
  archived_at IS NULL;

CREATE INDEX ix_tasks__board_column_position ON tasks (board_id, column_id, position)
WHERE
  archived_at IS NULL;

CREATE INDEX ix_tasks__org_active ON tasks (organization_id)
WHERE
  archived_at IS NULL;

CREATE INDEX ix_tasks__due_date ON tasks (due_date)
WHERE
  archived_at IS NULL
...
CREATE INDEX ix_tasks__archived ON tasks (organization_id, archived_at)
WHERE
...
CREATE INDEX ix_comments__task_created ON comments (task_id, created_at)
WHERE
  deleted_at IS NULL;
...
CREATE INDEX ix_attachments__task ON attachments (task_id)
WHERE
  archived_at IS NULL;

CREATE INDEX ix_notifications__recipient_unread ON notifications (recipient_user_id, created_at)
WHERE
  read_at IS NULL;
