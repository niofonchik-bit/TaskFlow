...
export default function App() {
    ...
    async function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`Не удалось сохранить файл: ${response.status}`);
            }

            const result = (await response.json()) as TestRecordResponse;

            setRecords((currentRecords) => currentRecords.map((record) => (record.id === result.data.id ? result.data : record)));
        } catch (requestError: unknown) {
            setError(requestError instanceof Error ? requestError.message : 'Не удалось сохранить файл');
        } finally {
    ...
    async function handleDownload(record: TestRecord) {
        ...
        try {
            ...
            setError(requestError instanceof Error ? requestError.message : 'Не удалось скачать файл');
        } finally {
    ...
    return (
        <main className='test_page'>
            ...
            {!isLoading && !error && records.length === 0 && <p>Тестовые записи отсутствуют.</p>}
                        ...
                        return (
                            ...
                            >
                                <div className='test_record_content'>
                                    ...
                                    {record.file && (
                                        <span>
                                            {record.file.fileName} · {formatFileSize(record.file.size)}
                                        </span>
                                    )}
                                    ...
                                    <div className='test_record_actions'>
                                        <button
                                            type='button'
                                            disabled={isUpdating}
                                            onClick={() => openFileDialog(record.id)}
                                        >
                                            {isUpdating ? 'Сохранение...' : record.file ? 'Изменить файл' : 'Добавить файл'}
                                        </button>
                                ...
                                <div className='test_record_preview'>
                                    {record.file?.previewUrl ? (
                                        ...
                                        <span>{record.file ? 'Предпросмотр недоступен' : 'Файл не выбран'}</span>
                                    )}
                                </div>
                            </li>
                        );
